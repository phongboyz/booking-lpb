...
class MemberComponent extends Component
{
    use WithFileUploads;
    public $users, $count;
    public $username, $fname, $lname, $password, $confirm_password, $phone, $role_id = 3, $branch_id, $profile, $profiles;
    ...
    public function render()
    {
        ...
        $this->users = User::whereAny(['username','phone'],'LIKE','%'.$this->search.'%')->where('role_id',3)->limit($this->dataQ)->get();
        $this->count = User::whereAny(['username','phone'],'LIKE','%'.$this->search.'%')->where('role_id',3)->limit($this->dataQ)->count();
        return view('livewire.backend.user.member-component')->layout('components.layouts.backend.app');
    }
    ...
    public function searchData(){
        if(!empty($this->dateS)){
            if($this->search){
                $this->users = User::whereDate('created_at', $this->dateS)->whereAny(['username','phone'],'LIKE','%'.$this->search.'%')->where('role_id',3)->limit($this->dataQ)->get();
                $this->count = User::whereDate('created_at', $this->dateS)->whereAny(['username','phone'],'LIKE','%'.$this->search.'%')->where('role_id',3)->limit($this->dataQ)->count();
            }else{
                // dd($this->dateS);
                $this->users = User::where('created_at', $this->dateS)->where('role_id',3)->limit($this->dataQ)->get();
                $this->count = User::where('created_at', $this->dateS)->where('role_id',3)->limit($this->dataQ)->count();
            }
        }else{
            $this->users = User::whereAny(['username','phone'],'LIKE','%'.$this->search.'%')->where('role_id',3)->limit($this->dataQ)->get();
            $this->count = User::whereAny(['username','phone'],'LIKE','%'.$this->search.'%')->where('role_id',3)->limit($this->dataQ)->count();
